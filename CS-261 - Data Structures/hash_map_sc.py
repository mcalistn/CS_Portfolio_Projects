...
# Due Date: 3/11/2022
# Description:  Implement the HashMap class with single linked lists (SLL) for chaining.
#               The HashMap class contains the following methods:
...
class HashMap:
    ...
    def get(self, key: str) -> object:
        ...
        """
        if self.buckets[self.hash_function(key) % self.capacity].contains(key) is not None:
            return self.buckets[self.hash_function(key) % self.capacity].contains(key).value
        else:
            return None
    ...
    def put(self, key: str, value: object) -> None:
        ...
        if not self.contains_key(key):
            self.buckets[self.hash_function(key) % self.capacity].insert(key, value)
            self.size += 1
        else:
            self.buckets[self.hash_function(key) % self.capacity].contains(key).value = value
    ...
    def remove(self, key: str) -> None:
        ...
        """
        if self.buckets[self.hash_function(key) % self.capacity].remove(key):
            self.size -= 1
    ...
    def contains_key(self, key: str) -> bool:
        ...
        """
        if self.buckets[self.hash_function(key) % self.capacity].contains(key) is None:
            return False
        else:
            return True
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        than 1, the method does nothing.
        """
        if new_capacity >= 1:
